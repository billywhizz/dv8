...
const IP_ADDRESS = env.IP_ADDRESS || '52.218.84.42'
...
async function run(parallel = '4') {
    await Promise.all((new Array(parseInt(parallel, 10))).fill(0).map((v, i) => fetch('whitespace.pdf', i)))
}

run(args[2]).catch(err => print(err.message))
