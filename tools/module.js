...
const { readFile, writeFile, mkdir } = require('./util.js')
const [name, className] = args.slice(2)
print(`name: ${name}, className: ${className}`)
const header = { text: '' }
compile('file.text = `' + readFile('./templates/module.h').read() + '`', '', ['name', 'className', 'file'], []).call({}, name, className, header)
const source = { text: '' }
compile('file.text = `' + readFile('./templates/module.cc').read() + '`', '', ['name', 'className', 'file'], []).call({}, name, className, source)
